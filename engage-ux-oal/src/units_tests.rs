...
fn metric_converts_using_dpcm() {
	// 1U (1cm) should map to dpcm pixels (dpi = dpcm * 2.54)
...
fn dpr_scales_pixels() {
	...
	// dpi=25.4 corresponds to dpcm=10
...
fn zero_and_negative_units() {
	let metrics = DeviceMetrics::new(25.4_f32, 1.0);
...
fn round_trip_px_to_units() {
	// imperial round-trip: use dpi directly
