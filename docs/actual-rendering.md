...
The current default backends (`WindowBackend`, `SoftbufferRenderer`, `TinySkiaRenderer`) are **simulation/stub implementations** used for testing:
...
- ✅ **Real window creation** using the project's platform windowing backend (project-maintained)
...
1. Create an application `EventLoop` (the embedding application supplies the primary event loop)
2. Create a platform `Window` using the project's windowing backend or platform APIs
3. Create a softbuffer `Surface` for the window (or a GPU-backed surface when available)
...
- Uses the project's windowing backend + softbuffer
- Creates real window
